...
public class DinnerModel extends Observable implements IDinnerModel {
	...
	public DinnerModel() {
		...
		Ingredient dish3ing2 = new Ingredient("sourdough starter", 1.5, "cups",
				6);
		...
		Ingredient dish4ing5 = new Ingredient(" golden caster sugar", 100, "g",
				6);
		...
		Ingredient dish6ing1 = new Ingredient("extra lean ground beef", 115,
				"g", 20);
		...
		Ingredient dish6ing7 = new Ingredient("crushed red pepper flakes", 0.6,
				"g", 3);
		Ingredient dish6ing8 = new Ingredient("Worcestershire sauce", 16, "ml",
				7);
		Ingredient dish6ing9 = new Ingredient("milk", 20, "ml", 4);
		Ingredient dish6ing10 = new Ingredient("grated Parmesan cheese", 5,
				"g", 8);
		Ingredient dish6ing11 = new Ingredient("seasoned bread crumbs", 115,
				"g", 4);
	...
	public DefaultTableModel getDishTableModel(String name) {
		...
		Iterator<Ingredient> ingIterator = this.getDishByName(name)
		...
		while (ingIterator.hasNext()) {
			...
			tableData[itrCount][1] = ingredient.getQuantity() + " "
					+ ingredient.getUnit();
	...
	// method that returns an a tableModel object with all ingredients needed
	// for the dinner
		...
		for (Dish dish : this.getFullMenu()) {
			for (Ingredient ingredient : dish.getIngredients()) {
				...
				if (this.getNumberOfGuests() > 1) {
					ingredientsData[i][1] = Math.round(ingredient.getQuantity()
							* this.getNumberOfGuests() * 100.0)
							/ 100.0 + " " + ingredient.getUnit();
				} else {
					ingredientsData[i][1] = ingredient.getQuantity() + " "
							+ ingredient.getUnit();
				}
